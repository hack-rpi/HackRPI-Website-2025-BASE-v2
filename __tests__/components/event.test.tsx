...
jest.mock('@/components/nav-bar/nav-bar', () => {
  return function MockNavBar({ showOnScroll }: { showOnScroll: boolean }) {
    return <div data-testid="nav-bar" data-show-on-scroll={showOnScroll}>NavBar Component</div>;
  };
});
...
jest.mock('@/components/themed-components/hackrpi-link', () => {
  return function MockHackRPILink({ 
    ...
    return (
      <a 
        data-testid={`hackrpi-link-${href.replace(/[^a-zA-Z0-9]/g, '-')}`} 
        href={href} 
        className={className}
      >
        {children}
...
describe('Event Page', () => {
  ...
  it('displays event location information', () => {
    ...
    expect(addressLink).toHaveAttribute('href', 'https://maps.google.com/?q=Darrin+Communications+Center+51+College+Ave+Troy+NY+12180');
  });
  ...
  it('displays parking information', () => {
    ...
    expect(screen.getByText('Parking is available at North Hall Parking Lot, 2-minute walk to Darrin Communications Center')).toBeInTheDocument();
