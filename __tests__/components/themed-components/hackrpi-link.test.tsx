...
jest.mock("next/link", () => {
	return ({ children, href, className, target, ...rest }: {
...
describe("HackRPILink Component", () => {
	...
	test("applies additional className from props", () => {
		render(<HackRPILink href="/test" className="custom-class">Custom Link</HackRPILink>);
		const link = screen.getByRole("link", { name: /Custom Link/i });
		expect(link).toHaveClass("custom-class");
	});

	test("applies target attribute when provided", () => {
		render(<HackRPILink href="/test" target="_blank">External Link</HackRPILink>);
	...
	test("combines default classes with custom class correctly", () => {
		render(<HackRPILink href="/test" className="px-4 py-2">Combined Classes</HackRPILink>);
